...
class BinPack(gym.Env, EzPickle):
    def __init__(self,capacity, items):
        ...
        self.items = items
        self.bins = items
        self.nBins = len(items)
        self.LB = ceil(sum(items)/capacity)
    ...
    def step(self, action):
        ...
        reward = 0 if not self.done() else self.solG.number_of_edges() /self.LB  
        state = [self.bins, self.G, self.capacity]
